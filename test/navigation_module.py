...
from decision_module import Decision
import sumolib
...
class Navigation():
    def __init__(self, agentID:str):
        ...
        self.rou_path = R'map\Map4_Tsinghua_Intersection\grid-map-traffic_VehicleOnly_200.rou.xml'
...
def get_sur_veh(vehID):
    vehs = []
    veh_infos = traci.vehicle.getContextSubscriptionResults(str(vehID))
    veh_info_dict = copy.deepcopy(veh_infos)
    ego_x, ego_y= veh_info_dict['0'][traci.constants.VAR_POSITION]
    for i, veh in enumerate(veh_info_dict):
        if veh != '0':
            ...
            vehs.append([x, y, v, phi, length, width])
    vehs_sort = sorted(vehs, key=lambda a: (a[0]-ego_x)**2+(a[1]-ego_y)**2)
    if len(vehs_sort) < 5:
        return vehs_sort
    else:
        return vehs_sort[:4]
...
def main():
    ...
    for i in range(int(num)):
        ...
        points = out_dict
        # print(type(points), len(points), type(points[0]), len(points[0]))
        ref = points[0][0][:, 0:10:2].T
        print(ref.shape)
        ref = ref.tolist()
        # print(ref)
        #
